...
void loop() {
  ...
  if(myRTC.hours==A_hour && myRTC.minutes==A_minute && AlarmIsActive==1 && myRTC.seconds >= 0 && myRTC.seconds <= 2){
    ...
    delay(100);
    lcd.clear();
    lcd.print("Buna iubita mea !!!"); //Message to show when the alarm is ringing
  ...
  if(myRTC.dayofmonth==28 && myRTC.hours==0 && myRTC.minutes==0 && myRTC.seconds==0)
  {
     LMA = 1;
  }    
  if(LMA==1){
    while(keypressedx == NO_KEY && LMA==1){
    tone(buzzer, 1000); //You can modify the tone or make your own sound
    delay(100);
    tone(buzzer, 2000);
    delay(100);
    lcd.clear();
    lcd.print("LA MULTI ANI BUBI !!"); //Message to show when the alarm is ringing
    lcd.setCursor(0, 1);
    ani=myRTC.year-2022;
    lcd.print(ani);
    lcd.print(" ani si ");
  ...
  //if(myRTC.dayofmonth==27 && myRTC.hours==23 && myRTC.minutes==59 && myRTC.seconds==59)
    //{
      //LMAVERF=1;
    //}   
